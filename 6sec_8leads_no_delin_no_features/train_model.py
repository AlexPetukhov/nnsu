...
def get_conv_model(filter_means, input_layer):
    for i in range(len(filter_means)):
        if i == 0:
            conv = L.Conv1D(filters=filter_means[i], kernel_size=16, padding='same', strides=5, activation='relu')(input_layer)
        else:
...
def model_many_diseases2():
    ...
    model_12 = get_conv_model(np.array([32, 64, 128, 64]), input_layer)
...
def main():
    ...
    callbacks = [
        ...
        ModelCheckpoint('model.{epoch:02d}-{val_loss:.2f}.h5', save_best_only=True, monitor='val_loss', mode='min'),
    ...
    model.save("models/model_6sec_noDelin_noFeatures_3")
    model.save("models/model_6sec_noDelin_noFeatures_3.h5")
